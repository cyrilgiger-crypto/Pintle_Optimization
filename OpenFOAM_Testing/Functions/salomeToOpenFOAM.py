...
debug=0      # Print Verbosity (0=silent => 3=chatty)
...
def exportToFoam(mesh,dirname='polyMesh'):
    ...
    for faceId in range(0,nrIntFaces):
        ...
        if ownedfaces > 1:
            ...
            neighbour[sId:eId+1]=list(map(neighbour.__getitem__,inds))
            faces[sId:eId+1]=list(map(faces.__getitem__,inds))
    ...
    for ind,gname in enumerate(grpNames):
        ...
        if "wall" in gname.lower():
            ...
            fileBoundary.write("\trotationAxis\t(1 0 0);\n")
            fileBoundary.write("\trotationCentre\t(0 0 0);\n")        
        else:
